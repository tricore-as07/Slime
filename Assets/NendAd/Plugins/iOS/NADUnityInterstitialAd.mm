...
- (void)dispatchShowResult:(NADInterstitialShowResult)result spotId:(NSInteger)spotId;
...
- (void)dispatchShowResult:(NADInterstitialShowResult)result spotId:(NSInteger)spotId
{
    ...
    NSString* message = [NSString stringWithFormat:@"%ld:%ld", (long)value, (long)spotId];
...
void _LoadInterstitialAd(int spotId, const char* apiKey, BOOL isOutputLog)
{
    ...
    [[NADInterstitial sharedInstance] loadAdWithSpotID:spotId apiKey:NADUnityCreateNSString(apiKey)];
}
...
void _ShowInterstitialAd(int spotId)
{
    ...
    if (spotId > 0) {
        result = [[NADInterstitial sharedInstance] showAdFromViewController:UnityGetGLViewController() spotID:spotId];
    } else {
        result = [[NADInterstitial sharedInstance] showAdFromViewController:UnityGetGLViewController()];
    }
    [[NADInterstitialEventDispatcher sharedDispatcher] dispatchShowResult:result spotId:spotId];
}
