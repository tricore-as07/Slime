...
﻿namespace NendUnityPlugin.Platform
{
	internal class NendAdNativeInterfaceFactory
	{
		...
		internal static NendAdInterstitialVideo CreateInterstitialVideoAd (int spotId, string apiKey)
		{
			#if UNITY_IOS && !UNITY_EDITOR
		...
		internal static NendAdRewardedVideo CreateRewardedVideoAd (int spotId, string apiKey)
		{
			#if UNITY_IOS && !UNITY_EDITOR
		...
		private class StubInterstitialVideo : NendAdInterstitialVideo
		{
			...
			{
				UnityEngine.Debug.Log ("AdTypeInternal");
				return 0; //enum value: Unknown
			}
