...
namespace NendUnityPlugin.Platform.iOS
{
	...
	internal class IOSInterstitialVideoAd : NendAdInterstitialVideo
	{
		...
		internal IOSInterstitialVideoAd (int spotId, string apiKey) : base ()
		{
		...
		{
			return _AdTypeInterstitial (m_iOSInterstitialVideoAdPtr);
		}
		...
		private static extern IntPtr _CreateInterstitialVideoAd (int spotId, string apiKey);

		[DllImport ("__Internal")]
		private static extern int _AdTypeInterstitial (IntPtr iOSVideoPtr);
		...
		[DllImport ("__Internal")]
		private static extern void _AddFallbackFullboard (IntPtr iOSVideoPtr, int spotId, string apiKey, float r, float g, float b, float a);
